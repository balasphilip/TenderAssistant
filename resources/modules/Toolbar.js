    ...
    var toolbarInit = function (type) {
        type
            .findTextareaById()
            .addToolbar()
            .addEvList();
        //Here comes the part which add the event handlers for bid template list
        window.addEventListener("message", type.receiveMessage.bind(type), false);
    };

    if (Odesk.isApplicationPage()) {
       toolbarInit(new Toolbar('coverLetter'));
    } else if (Elance.isApplicationPage()) {
        toolbarInit(new Toolbar('bid_exp-plaintext'));
    }
